...
#
# Env knobs (identical to serve_agg_sglang.sh): MODEL, TP, MAX_MODEL_LEN, OFFLOAD_GB,
...
ENGINE_TAG="sglang_tp${TP}_ep${EP}"; { [ "$OFFLOAD_GB" -gt 0 ] 2>/dev/null && ENGINE_TAG="${ENGINE_TAG}_offload${OFFLOAD_GB}"; } || true
...
echo "[serve-sglang-ep] launch_server $MODEL  TP=$TP  EP=$EP  moe-runner=${MOE_RUNNER:-auto}  radix-cache(prefix)=ON  offload_gb=$OFFLOAD_GB"
...
python3 sweep.py \
  ...
  --result-dir "$RESULT_DIR" --title "MiniMax-M2.5 agg SGLang TP$TP EP$EP offload=${OFFLOAD_GB}GB"
