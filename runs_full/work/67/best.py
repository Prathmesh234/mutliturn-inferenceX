...
def _linear_act_kernel(x_ptr, w_ptr, b_ptr, out_ptr,
    ...
    for k in range(K):
        ...
        w_k = tl.load(w_ptr + offs_n * K + k)  # w is [N, K], row n
...
class LinearBlockNew(nn.Module):
    ...
    def forward(self, x):
        ...
        # Determine if we can fuse activation. Only fuse when no norm.
        fuse_act = self.norm is None and self._act_name in ('relu', 'lrelu', 'tanh', 'none')
        act_code = 0
        neg_slope = 0.0
        if fuse_act:
            if self._act_name == 'relu':
                act_code = 1
            elif self._act_name == 'lrelu':
                act_code = 2
                neg_slope = 0.2
            elif self._act_name == 'tanh':
                act_code = 3

        BLOCK_M = 64
        grid = (triton.cdiv(M, BLOCK_M),)
        _linear_act_kernel[grid](x_flat, self.fc.weight, self.fc.bias, out,
                                 M, K, N, act_code, neg_slope,
