...
class DecoderNew(nn.Module):
    ...
    def forward(self, x):
        x = deconv2d(x, self.deconv1.weight, 1, 0, True, BLOCK_M=16, BLOCK_N=32, BLOCK_K=16, num_warps=1)
        x = deconv2d(x, self.deconv2.weight, 2, 1, True, BLOCK_M=16, BLOCK_N=32, BLOCK_K=128, num_warps=1)
        x = deconv2d(x, self.deconv3.weight, 2, 1, False, BLOCK_M=16, BLOCK_N=16, BLOCK_K=64, num_warps=1)
        return x
