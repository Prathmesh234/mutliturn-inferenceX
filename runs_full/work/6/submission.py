...
@triton.jit
def _behler_kernel(cos_ptr, out_ptr, n_elements, coef, col_pos, col_neg,
                   ZETA: tl.constexpr, NCOL: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    ...
    pos = a
    neg = b
    for _ in tl.static_range(ZETA - 1):
        pos = pos * a
        neg = neg * b
    pos = coef * pos
    neg = coef * neg
...
class BehlerAngularNew(nn.Module):
    ...
    def forward(self, cos_theta):
        ...
        for i, zeta in enumerate(zetas):
            coef = float(2 ** (1 - zeta))
            _behler_kernel[grid](c, out, n, coef, i, L + i,
                                 ZETA=int(zeta), NCOL=ncol,
