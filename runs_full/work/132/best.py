...
def _kernel(
    ...
    for ic in range(IC):
        for k in range(K):
            ...
            xptr = x_ptr + n * C_in * L + ic * L + in_idx
            xval = tl.load(xptr, mask=valid & ol_mask, other=0.0)
            wval = tl.load(w_ptr + oc * IC * K + ic * K + k)
            acc += xval * wval

    acc += tl.load(b_ptr + oc)
    out_off = n * OC * Lout + oc * Lout + ol
    tl.store(out_ptr + out_off, acc, mask=ol_mask)
...
class ResizeConv1dNew(nn.Module):
    ...
    def forward(self, x):
        ...
        _kernel[grid](
            ...
            N, C, L, Lup, Lout, OC,
            IC=IC, K=K, PAD=PAD, BLOCK_L=BLOCK_L,
            num_warps=1,
        )
        return out
