...
# torch's fused GPU slogdet/det codegen is broken on this sm_103 device (nvrtc
# "unsupported cast"). Compute the log-abs-determinant with a small custom
# Triton LU kernel instead -- same math, fully on-device, cheap.
@triton.jit
...
def _tri_logabsdet(A):
    ...
    _logabsdet_kernel[(1,)](A.contiguous(), out, C, BLOCK=BLOCK)
    return out.reshape(())
...
class InvConvNew(nn.Module):
    ...
    def forward(self, x, sldj, reverse=False):
        ...
        _conv1x1_kernel[grid](x, weight, z, C=C, HW=HW, BLOCK=BLOCK, num_warps=4)
        return z, sldj
