...
def _full_kernel(xq, xk, xv, wq, wk, wv, wo, bq, bk, bv, bo, out_ptr,
    ...
    # q projection for token (b, i): (P,)
    xqr = tl.load(xq + (b * SEQ + i) * DM + offs_p, mask=p_mask, other=0.0)
    qp = tl.sum(xqr[None, :] * wqm, axis=1) + bqv  # (P,)
    q = tl.reshape(qp, (BLOCK_H, BLOCK_DK))

    # k, v projections for all tokens j of batch b: (BLOCK_N, P)
    ...
    kp = tl.sum(xkr[:, None, :] * wkm[None, :, :], axis=2) + bkv[None, :]  # (N, P)
    vp = tl.sum(xvr[:, None, :] * wvm[None, :, :], axis=2) + bvv[None, :]  # (N, P)
    k = tl.reshape(kp, (BLOCK_N, BLOCK_H, BLOCK_DK))
    v = tl.reshape(vp, (BLOCK_N, BLOCK_H, BLOCK_DK))

    # scores[h, j] = sum_d q[h,d]*k[j,h,d]
    s = tl.sum(q[None, :, :] * k, axis=2)  # (N, H)
    scores = tl.trans(s) * scale  # (H, N)
    scores = tl.where(n_mask[None, :], scores, -float('inf'))
    mx = tl.max(scores, axis=1)[:, None]
    pw = tl.exp(scores - mx)
    pw = pw / tl.sum(pw, axis=1)[:, None]  # (H, N)
    pt = tl.trans(pw)  # (N, H)
    o_hd = tl.sum(pt[:, :, None] * v, axis=0)  # (H, DK)
    concat = tl.reshape(o_hd, (P,))  # (P,)
    ...
    res = tl.sum(concat[None, :] * wom, axis=1) + bov  # (P,)
...
class MultiHeadAttentionNew(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        _full_kernel[(bs * seq,)](
            ...
            BLOCK_H=BLOCK_H, BLOCK_N=BLOCK_N, BLOCK_DK=BLOCK_DK, P=P, num_warps=4)
