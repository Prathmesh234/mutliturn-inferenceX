...
def _ffn_kernel(x_ptr, w1_ptr, b1_ptr, w2_ptr, b2_ptr,
    ...
    # w1 : [DHID, DIN]  (nn.Linear weight layout [out, in])
    ...
    # h[m,j] = sum_i x[m,i]*w1[j,i] + b1[j]  -> [BM, DHID]
    ...
    # y[m,k] = sum_j h[m,j]*w2[k,j] + b2[k]  -> [BM, DIN]
    y = tl.sum(h[:, None, :] * w2[None, :, :], axis=2) + b2[None, :]

    # residual
    y = y + x

    # layernorm over d_in (masked)
    cnt = d_in
...
class PositionwiseFeedForwardNew(nn.Module):
    """ A two-feed-forward-layer module (Triton fused) """
