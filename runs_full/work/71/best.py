...
def _fused_kernel(QF, KF, WQ, WK, BQ, BK, S, PW, PB, OUT,
    ...
    bb = offs_hb % B
    hh = offs_hb // B
    wrow = hh[:, None] * D + offs_d[None, :]          # [HB, D]

    # qx[hb,d] = sum_e QF[(bb*Lq+i)*E+e]*WQ[(hh*D+d)*E+e] + BQ
    qflat = tl.load(QF + (bb[:, None] * Lq + i) * E + offs_e[None, :],
                    mask=mask_hb[:, None] & mask_e[None, :], other=0.0)  # [HB,E]
    wq = tl.load(WQ + wrow[:, :, None] * E + offs_e[None, None, :],
                 mask=mask_hb[:, None, None] & mask_d[None, :, None] & mask_e[None, None, :],
                 other=0.0)  # [HB,D,E]
    bq = tl.load(BQ + wrow, mask=mask_hb[:, None] & mask_d[None, :], other=0.0)  # [HB,D]
    qx = tl.sum(qflat[:, None, :] * wq, axis=2) + bq  # [HB,D]

    # kx[hb,j,d] = sum_e KF[(bb*Lk+j)*E+e]*WK[(hh*D+d)*E+e] + BK
    ...
    wk = tl.load(WK + wrow[:, :, None] * E + offs_e[None, None, :],
                 mask=mask_hb[:, None, None] & mask_d[None, :, None] & mask_e[None, None, :],
                 other=0.0)  # [HB,D,E]
    bk = tl.load(BK + wrow, mask=mask_hb[:, None] & mask_d[None, :], other=0.0)  # [HB,D]
    # [HB,Lk,1,E]*[HB,1,D,E] -> sum over E -> [HB,Lk,D]
    kfull = tl.sum(kflat[:, :, None, :] * wk[:, None, :, :], axis=3) + bk[:, None, :]  # [HB,Lk,D]

    # score[hb,j] = sum_d qx[hb,d]*kfull[hb,j,d]
    score = tl.sum(qx[:, None, :] * kfull, axis=2) * scale  # [HB,Lk]
    ...
    out = tl.sum(sm[:, :, None] * kfull, axis=1)  # [HB,D]
    ...
    pw = tl.load(PW + offs_o[None, :, None] * HD + hh[:, None, None] * D + offs_d[None, None, :],
                 ...
                 other=0.0)  # [HB,O,D]
    partial = tl.sum(out[:, None, :] * pw, axis=2)  # [HB,O]
    sel = (offs_b[:, None] == bb[None, :]) & mask_b[:, None] & mask_hb[None, :]
    result = tl.sum(tl.where(sel[:, :, None], partial[None, :, :], 0.0), axis=1)  # [B,O]
