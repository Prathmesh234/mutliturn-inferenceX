...
@triton.jit
def _sce_kernel(x_ptr, t_ptr, acc_ptr, R, C, inner,
...
class SmoothCrossEntropyLossNew(nn.Module):
    ...
    def forward(self, input, target):
        ...
        acc = torch.zeros(1, dtype=torch.float32, device=input.device)
        BLOCK_C = triton.next_power_of_2(C)
        BLOCK_R = 64
        grid = (triton.cdiv(R, BLOCK_R),)
        _sce_kernel[grid](input, target, acc, R, C, inner,
                          BLOCK_R=BLOCK_R, BLOCK_C=BLOCK_C, num_warps=4)

        out = acc[0] if not self.size_average else acc[0] / R
