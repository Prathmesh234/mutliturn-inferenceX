...
def _biatt_kernel(
    ...
    input_dot = tl.sum(inp * w_in[None, :], axis=1)      # (BI,)
    memory_dot = tl.sum(mem * w_mem[None, :], axis=1)    # (BM,)
    input_scaled = inp * scale[None, :]                  # (BI, BD)
    cross = tl.sum(input_scaled[:, None, :] * mem[None, :, :], axis=2)  # (BI,BM)

    att = input_dot[:, None] + memory_dot[None, :] + cross  # (BI, BM)
    ...
    # softmax over memory dim
    m1 = tl.max(att, axis=1)                  # (BI,)
    e1 = tl.exp(att - m1[:, None])
    denom1 = tl.sum(e1, axis=1)               # (BI,)
    w1 = e1 / denom1[:, None]                 # (BI, BM)
    output_one = tl.sum(w1[:, :, None] * mem[None, :, :], axis=1)  # (BI, BD)

    # weight_two : softmax over input dim of att.max(dim=-1)
    valid_i = offs_i < I
    ...
    w2 = e2 / denom2                          # (BI,)
    output_two = tl.sum(w2[:, None] * inp, axis=0)  # (BD,)
