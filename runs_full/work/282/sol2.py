...
def _focal_strided(x_ptr, t_ptr, out_ptr, n,
                   shape_ptr, sx_ptr, st_ptr,
                   alpha, gamma, eps,
                   ND: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0)
    offs = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offs < n

    rem = offs
    xoff = tl.zeros((BLOCK_SIZE,), dtype=tl.int32)
    toff = tl.zeros((BLOCK_SIZE,), dtype=tl.int32)
    for i in tl.static_range(ND):
        d = ND - 1 - i
        dim = tl.load(shape_ptr + d)
        idx_d = rem % dim
        rem = rem // dim
        xoff += idx_d * tl.load(sx_ptr + d)
        toff += idx_d * tl.load(st_ptr + d)
...
class BinaryFocalLossWithLogitsNew(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        sx = inp.broadcast_to(out_shape).stride()
        st = tgt.broadcast_to(out_shape).stride()

        dev = inp.device
        shape_t = torch.tensor(out_shape, device=dev, dtype=torch.int32)
        sx_t = torch.tensor(sx, device=dev, dtype=torch.int32)
        st_t = torch.tensor(st, device=dev, dtype=torch.int32)

        out = torch.empty(out_shape, device=dev, dtype=torch.float32)
        n = out.numel()
        BLOCK_SIZE = 1024
        grid = (triton.cdiv(n, BLOCK_SIZE),)
        _focal_strided[grid](inp, tgt_base, out, n, shape_t, sx_t, st_t,
                             float(self.alpha), float(self.gamma),
                             float(self.eps), ND=nd, BLOCK_SIZE=BLOCK_SIZE,
                             num_warps=4)
