...
def _bva_kernel(
    ...
    # load x[n]: [BL, BC]
    x = tl.load(x_ptr + n * L * C + offs_l[:, None] * C + offs_c[None, :],
                mask=mask_l[:, None] & mask_c[None, :], other=0.0)

    # weights [H, C]
    wt = tl.load(wt_ptr + offs_h[:, None] * C + offs_c[None, :],
                 mask=mask_h[:, None] & mask_c[None, :], other=0.0)
    wph = tl.load(wph_ptr + offs_h[:, None] * C + offs_c[None, :],
                  mask=mask_h[:, None] & mask_c[None, :], other=0.0)
    wpsi = tl.load(wpsi_ptr + offs_h[:, None] * C + offs_c[None, :],
                   mask=mask_h[:, None] & mask_c[None, :], other=0.0)
    ...
    # x_t[l,h] = sum_c x[l,c]*wt[h,c] + bt[h]  -> [BL, BH]
    x_t = tl.sum(x[:, None, :] * wt[None, :, :], axis=2) + bt[None, :]
    x_ph = tl.sum(x[:, None, :] * wph[None, :, :], axis=2) + bph[None, :]
    x_psi = tl.sum(x[:, None, :] * wpsi[None, :, :], axis=2) + bpsi[None, :]

    # attn[l,m] = sum_h x_ph[l,h]*x_t[m,h]  -> [BL, BL]
    attn = tl.sum(x_ph[:, None, :] * x_t[None, :, :], axis=2)
    # softmax over m (axis=1), mask invalid m
    attn = tl.where(mask_l[None, :], attn, float("-inf"))
    attn = attn - tl.max(attn, axis=1)[:, None]
    attn = tl.exp(attn)
    attn = attn / tl.sum(attn, axis=1)[:, None]

    # x_add[l,h] = sum_m attn[l,m]*x_psi[m,h] -> [BL, BH]
    x_add = tl.sum(attn[:, :, None] * x_psi[None, :, :], axis=1)

    # recover1: [C2, H]
    wr1 = tl.load(wr1_ptr + offs_c2[:, None] * H + offs_h[None, :],
                  mask=mask_c2[:, None] & mask_h[None, :], other=0.0)
    br1 = tl.load(br1_ptr + offs_c2, mask=mask_c2, other=0.0)
    r1 = tl.sum(x_add[:, None, :] * wr1[None, :, :], axis=2) + br1[None, :]  # [BL, BC2]
    r1 = tl.where(r1 > 0, r1, 0.2 * r1)

    # recover2: [C, C2]
    wr2 = tl.load(wr2_ptr + offs_c[:, None] * C2 + offs_c2[None, :],
                  mask=mask_c[:, None] & mask_c2[None, :], other=0.0)
    br2 = tl.load(br2_ptr + offs_c, mask=mask_c, other=0.0)
    r2 = tl.sum(r1[:, None, :] * wr2[None, :, :], axis=2) + br2[None, :]  # [BL, BC]
    r2 = (tl.exp(2 * r2) - 1) / (tl.exp(2 * r2) + 1)

    out = x + r2
    tl.store(out_ptr + n * L * C + offs_l[:, None] * C + offs_c[None, :],
             out, mask=mask_l[:, None] & mask_c[None, :])
